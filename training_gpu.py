...
if __name__ == "__main__":
    ...
    base_env = OvercookedEnv.from_mdp(mdp=base_mdp, info_level=0, horizon=horizon)
    env = Overcooked(base_env=base_env, featurize_fn=base_env.featurize_state_mdp)

    inp_shape = env.observation_space._shape
    ...
    old_policy = Policy(inp_shape=inp_shape, num_actions=Action.NUM_ACTIONS, optimizer=None)
    ...
    for episode in range(1,NUM_EPISODES+1):
        ...
        while not done:
            action_1 = agent_1.action(obs)
            action_2 = agent_2.action(obs)
            ...
            episode_new_observations.append(new_obs)
