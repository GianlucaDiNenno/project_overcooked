...
def parse_args():
    ...
    parser.add_argument("--exp-name", type=str, default="GEN_exp_two_policies", help="the name of this experiment")
    parser.add_argument("--seed", type=int, default=42, help="set the seed for reproducibility of the experiment")
    parser.add_argument("--num-episodes", type=int, default=2000, help="number of episodes to train the agent on")
    parser.add_argument("--num-epochs", type=int, default=4, help="number of epochs to train the agent on with SGD")
    parser.add_argument("--batch-size", type=int, default=128, help="batch size of the training with SGD")
    ...
    parser.add_argument("--saving_file", type=str, default="GEN_exp_two_policies", help="name of the file in which to save the weights")
...
def load_weights():
    """
    Load the weights of the neural networks from a file.
    If the file does not exist, the weights are initialized randomly.
    """
    if LOAD_WEIGHTS:
...
def save_weights():
    ...
    actor_1.save_weights(PATH_ACTOR_1)
    actor_2.save_weights(PATH_ACTOR_2)
...
if __name__ == "__main__":
    ...
    PATH_CRITIC = os.path.join("weights", "critic", "critic_" + EXP_NAME + ".weights.h5")
    ...
    horizon = 400
    layouts_name = ["cramped_room", "asymmetric_advantages", "cramped_room", "asymmetric_advantages", "cramped_room"]
    gen_env = GeneralizedOvercooked(layouts=layouts_name)

    agent_obs_shape = gen_env.observation_space.shape[0]
    new_inp_shape = (agent_obs_shape*2 + gen_env.num_layouts,)

    # defining the two actors and the critic
    actor_1 = Policy(
    ...
    for episode in range(1,NUM_EPISODES+1):
        ...
        events_to_reward = {
            ...
            'viable_onion_potting': VIABLE_POTTING_REWARD,
        ...
        for event, reward in events_to_reward.items():
            ...
            for agent in range(len(timesteps_event)):
                for timestep in timesteps_event[agent]:
                    ...
                    if timestep < len(episode_rewards): 
        ...
        for epoch in range(1, NUM_EPOCHS+1):
            ...
            for obs_batch, actions_batch, advantages_batch, value_targets_batch in dataset:
                #performing a training step for both actors and the critic
                actor_1.batch_train_PPO(advantages_batch, obs_batch, actions_batch, old_policy_1)
                actor_2.batch_train_PPO(advantages_batch, obs_batch, actions_batch, old_policy_2)                
